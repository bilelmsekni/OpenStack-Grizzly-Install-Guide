==========================================================
  OpenStack Havana Single Node Install Guide with Linux Bridge
==========================================================

:Keywords: Single node OpenStack, Havana, Neutron, Nova, Keystone, Glance, Horizon, Cinder, LinuxBridge, KVM, Ubuntu Server 12.04 (64 bits).

Credits
===============================

This guide is LARGELY derived from install guides by `Bilel Msekni <http://www.linkedin.com/profile/view?id=136237741&trk=tab_pro>`_ <bilel.msekni@gmail.com> && contributors

The instructions here in have been modded for Havana
...
* Fill up the keystone database using the two scripts available in the `Scripts folder <https://github.com/mseknibilel/OpenStack-Havana-Install-Guide/tree/master/KeystoneScripts>`_ of this git repository::

   #Modify the HOST_IP and HOST_IP_EXT variables before executing the scripts

   wget https://raw.github.com/mseknibilel/OpenStack-Havana-Install-Guide/master/KeystoneScripts/keystone_basic.sh
   wget https://raw.github.com/mseknibilel/OpenStack-Havana-Install-Guide/master/KeystoneScripts/keystone_endpoints_basic.sh
...
* Edit the LinuxBridge plugin config file /etc/neutron/plugins/linuxbridge/linuxbridge_conf.ini with:: 

   # under [DATABASE] section  
   sql_connection = mysql://neutronUser:neutronPass@10.10.100.51/neutron
...
* Restart all neutron services::
   ...
   for i in $( ls /etc/init.d/neutron-* | xargs -n 1 basename ); do sudo service $i restart; done
...
* Check the status of all nova-services::
   ...
   for i in $( ls /etc/init.d/cinder-* | xargs -n 1 basename ); do sudo service $i status; done
...
* Modify the /etc/nova/nova.conf like this::
   ...
   neutron_url=http://10.10.100.51:9696
   neutron_auth_strategy=keystone
   neutron_admin_tenant_name=service
   neutron_admin_username=neutron
   neutron_admin_password=service_pass
   neutron_admin_auth_url=http://10.10.100.51:35357/v2.0
...
* Restart nova-* services::
   ...
   for i in $( ls /etc/init.d/nova-* | xargs -n 1 basename ); do sudo service $i restart; done
...
* Create a new subnet inside the new tenant network::
   ...
   neutron subnet-create --tenant-id $put_id_of_project_one net_proj_one 50.50.1.0/24
...
OpenStack Havana Install Guide is licensed under a Creative Commons Attribution 3.0 Unported License.
